...
case class Parser(root: ElementStartNode) {
  ...
  def annosIsEmpty(): Unit = {
    if (annos.nonEmpty) {
      ...
      throw new SyntaxException(
        s"annotations are not presented at correct position",
        lineCol)
    }
  }
  ...
  def modifiersIsEmpty(): Unit = {
    if (modifiers.nonEmpty) {
      ...
      throw new SyntaxException(
        s"modifiers are not presented at correct position",
        lineCol)
    }
  }
    ...
    if (isParsingTry) {
      ...
      while (current.isInstanceOf[Element]) {
        ...
        exp match {
          ...
          case _ =>
            throw UnexpectedTokenException("exception type",
                                           exp.toString,
                                           exp.lineCol)
        }
        current match {
      ...
      current match {
        case node: ElementStartNode =>
          return Catch(exceptionTypes,
                       ...
                       lineCol)
        case _ =>
          throw new UnexpectedTokenException(current.toString(),
                                             current.lineCol)
      }
    } else {
      current match {
            ...
            case "static" => {
              ...
              e.next match {
                ...
                case ee: Element =>
                  ...
                  val stmt = parseStatement()
                  if (stmt == null)
                    throw UnexpectedTokenException("a valid statement",
                                                   ee.toString,
                                                   ee.lineCol)
    ...
    loop.breakable {
      while (current.isInstanceOf[Element]
        ...
        if (condition == null)
          isLast = true
        val pair =
          IfPair(condition, if (list == null) List() else list, ifPairLineCol)
        pairs :+= pair
  ...
  def parseFor(): ForStatement = {
    ...
    if (!varElem.isValidName)
      throw UnexpectedTokenException("valid variable name",
                                     varName,
                                     current.lineCol)
  ...
  def parseDo(): WhileStatement = {
    ...
    current match {
      ...
      case _ => {
        throw UnexpectedTokenException("while",
                                       current.toString,
                                       current.lineCol)
      }
    }
    ...
    current match {
      case e: ElementStartNode =>
        WhileStatement(condition,
                       ...
                       lineCol)
      case _ =>
        throw UnexpectedTokenException("while body",
                                       current.toString,
                                       current.lineCol)
    }
  }
    ...
    current match {
      case e: Element => {
        ...
        if (e.isValidName) {
                  ...
                  current match {
                    case e2: ElementStartNode => {
                      ...
                      list.foreach {
                        case stmt: Access => {
                          if (mustHaveInit)
                            throw new SyntaxException("parameter with init",
                                                      stmt.lineCol)
                          if (stmt.expression != null)
                            throw UnexpectedTokenException("param def",
                                                           stmt.toString,
                                                           stmt.lineCol)
                          else {
                            val v = VariableDef(stmt.name,
                                                Set(),
                                                null,
                                                null,
                                                annos,
          ...
          throw UnexpectedTokenException("valid class name",
                                         name,
                                         current.lineCol)
      }
      case _ =>
        throw UnexpectedTokenException("class name",
                                       current.toString,
                                       current.lineCol)
    }
  }
    ...
    current match {
      case e: Element => {
        ...
        if (e.isValidName) {
                    ...
                    case e: Element => {
                      if (e.isValidName) {
                        ...
                        v match {
                          ...
                          case _ =>
                            throw UnexpectedTokenException("super interface",
                                                           v.toString,
                                                           v.lineCol)
                        }
                        current match {
                          case node: EndingNode
                              if node.nodeType == EndingNode.STRONG =>
                            nextNode(true)
                          case _ =>
                            loop.break
                        }
          ...
          throw UnexpectedTokenException("valid interface name",
                                         current.toString,
                                         current.lineCol)
      }
      case _ =>
        throw UnexpectedTokenException("interface name",
                                       current.toString,
                                       current.lineCol)
    }
  }
  ...
  def parseTry(): Try = {
    ...
    current match {
      case e: EndingNode
          if e.next.isInstanceOf[Element] && (e.next
                      ...
                      case node: ElementStartNode =>
                        ...
                        catches.foreach {
                          case stmt: Catch =>
                            catchList += stmt
                          case stmt =>
                            throw UnexpectedTokenException("catch statements",
                                                           stmt.toString,
                                                           stmt.lineCol)
                        }
                        nextNode(true)
  ...
  def parseAnno(): Unit = {
    ...
    exp match {
        ...
        e.args.foreach {
          case v: VariableDef => {
            if (v.init.isInstanceOf[Literal]
              ...
              throw UnexpectedTokenException("literal or arry",
                                             v.toString,
                                             v.lineCol)
            }
          }
      ...
      case _ =>
        throw UnexpectedTokenException("annotation definition",
                                       exp.toString,
                                       exp.lineCol)
    }
    annos += anno
    ...
    current match {
      case e: ElementStartNode => {
        ...
        pkgNode match {
          ...
          case _ =>
            throw UnexpectedTokenException("package",
                                           pkgNode.toString,
                                           pkgNode.lineCol)
        }
      }
    ...
    current match {
      case e: ElementStartNode => {
        ...
        statements.foreach {
          case s: Access => {
            ...
            if (s.name == "_") {
              s.expression match {
                ...
                case _ =>
                  detail =
                    ImportDetail(null, s.expression.asInstanceOf[Access], true)
              }
            } else
          ...
          case _ =>
            throw UnexpectedTokenException("import statements",
                                           current.toString,
                                           current.lineCol)
        }
      }
  ...
  def parseMethodDefType(): MethodStatement = {
    ...
    current match {
      case e: Element if e.content == "=" => {
        ...
        if (!(current == null || current.isInstanceOf[EndingNode])) {
          throw UnexpectedTokenException("EndingNode",
                                         current.toString,
                                         current.lineCol)
        }
  ...
  def parseMethodDefVariables(variableList: ListBuffer[VariableDef],
        ...
        for (stmt <- statements) {
          stmt match {
            case s: Access if s.expression == null => {
              if (mustHaveInit)
                throw new SyntaxException("parameter with init value",
                                          s.lineCol)
  ...
  def parseOperatorLikeInvocation(): Unit = {
    ...
    current.next match {
      case e: Element
          if !isParsingMap || e.asInstanceOf[Element].content != ":" => {
        if (!binVarOps.empty()) {
  ...
  def parseIndexAccess(): Unit = {
    ...
    current match {
      ...
      case _ =>
        ...
        val stmts = parseElemStart(current.asInstanceOf[ElementStartNode],
                                   addUsedNames = true,
                                   Set(),
                                   parseMap = false,
                                   parseTry = false)
        ...
        list.foreach {
          case a: Access =>
            if (a.expression == null) {
              val v =
                VariableDef(a.name, Set(), null, null, annos, LineCol.SYNTHETIC)
              annos = Set()
              variableDefList :+= v
              set += a.name
            } else
  ...
  def parseExpMap(startNode: ElementStartNode): MapExp = {
    val stmts = parseElemStart(startNode,
                               addUsedNames = true,
                               Set(),
                               parseMap = true,
                               parseTry = false)
    ...
    expr match {
      case v: Access =>
        if (v.expression != null)
          throw UnexpectedTokenException("variable definition",
                                         v.toString,
                                         v.lineCol)
        val name = v.name
