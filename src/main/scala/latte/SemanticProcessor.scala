...
class SemanticProcessor(
    mapOfStatements: mutable.HashMap[String, List[Statement]]) {
  ...
  private val originalClasses: mutable.HashMap[String, ClassStatement] =
    mutable.HashMap()
  private val originalInterfaces: mutable.HashMap[String, InterfaceStatement] =
    mutable.HashMap()
  private val methodToStatements
    : mutable.HashMap[SMethodDef, ListBuffer[Statement]] = mutable.HashMap()
  private val fileNameToImport: mutable.HashMap[String, ListBuffer[Import]] =
    mutable.HashMap()
  ...
  def parseValueFromExpression(init: Expression,
                               typeDef: STypeDef,
                               value: SemanticScope): Value = ???

  def parseField(variableDef: VariableDef,
                 sClassDef: STypeDef,
                 imports: ListBuffer[Import],
                 PARSING_CLASS: Int,
                 bool: Boolean): Unit = ???

  def parseMethod(stmt: MethodStatement,
                  i: Int,
                  sClassDef: STypeDef,
  ...
  def parseStatement(statement: Statement,
                     voidType: VoidType,
                     constructorScope: SemanticScope,
                     statements: ListBuffer[Instruction],
                     exceptionTables: ListBuffer[ExceptionTable],
                     dontParseMethod: Boolean): Unit = ???

  def parseMethod1(m: SMethodDef,
                   statements: ListBuffer[Statement],
                   scope: SemanticScope) = ???

  def parse: mutable.HashSet[STypeDef] = {
    val fileNameToClassDef
      : mutable.HashMap[String, ListBuffer[ClassStatement]] = mutable.HashMap()
    val fileNameToInterfaceDef
      : mutable.HashMap[String, ListBuffer[InterfaceStatement]] =
      mutable.HashMap()
    val fileNameToPackageName: mutable.HashMap[String, String] =
      mutable.HashMap()
    mapOfStatements.foreach { x =>
      ...
      classDefs.foreach { c =>
        ...
        if (types.contains(className))
          throw new SyntaxException(s"duplicate type names $className",
                                    c.lineCol)

        val sClassDef = SClassDef(c.lineCol)
        sClassDef.fullName = className
        sClassDef.pkg =
          if (pkg.endsWith(".")) pkg.substring(0, pkg.length - 1) else pkg
      ...
      interfaceDefs.foreach { i =>
        ...
        if (types.contains(interfaceName))
          throw new SyntaxException(s"duplicate type names $interfaceName",
                                    i.lineCol)
    ...
    mapOfStatements.foreach { m =>
      ...
      val interfaceDefs: ListBuffer[InterfaceStatement] =
        fileNameToInterfaceDef(m._1)
      ...
      loop.breakable {
        classDefs.foreach { c =>
            ...
            if (c.superWithOutInvocation.isEmpty) {
              ...
              tmp match {
                ...
                case s: SInterfaceDef =>
                  ...
                  sClassDef.parent =
                    getTypeWithName("java.lang.Object", c.lineCol)
                      .asInstanceOf[SClassDef]
                case _ =>
          ...
          for (i <- c.params.size until generateIndex by -1) {
            ...
            if (lastConstructor != null) {
              val invoke = InvokeSpecial(This(sClassDef),
                                         lastConstructor,
                                         LineCol.SYNTHETIC)
        ...
        for (interfaceDef <- interfaceDefs) {
          ...
          for (access <- interfaceDef.superInterfaces) {
            val superInterface =
              getTypeWithAccess(access, imports).asInstanceOf[SInterfaceDef]
            sInterfaceDef.superInterfaces += superInterface
          }

          parseAnnos(interfaceDef.annos,
                     sInterfaceDef,
                     imports,
                     ElementType.TYPE)
          ...
          interfaceDef.statements.foreach {
            ...
            case stmt @ _ =>
              throw new SyntaxException("interfaces don't have initiators",
                                        stmt.lineCol)
          }
    ...
    typeDefSet.foreach {
      case sTypeDef: SClassDef =>
        ...
        while (parent != null) {
          ...
          if (parent == sTypeDef)
            throw new SyntaxException(
              "circular inheritance " + circularRecorder,
              LineCol.SYNTHETIC)
      ...
      loop.breakable {
        s match {
          case sc: SClassDef => {
            ...
            while (parent != null && parent.modifiers.contains(
                     SModifier.ABSTRACT)) {
              if (parent.parent != null && parent.parent.modifiers.contains(
                    SModifier.ABSTRACT))
              ...
              for (m <- parent.methods) {
                if (m.modifiers.contains(SModifier.ABSTRACT)) {
                  if (m.overRidden.isEmpty)
                    throw new SyntaxException(m + s" is not overridden in $sc",
                                              sc.lineCol)
                }
              }
              parent = parent.parent
      ...
      sTypeDef match {
        case sClassDef: SClassDef => {
            ...
            loop.breakable {
              for (con <- parent.constructors) {
                if (con.parameters.isEmpty) {
                  invokeSpecial =
                    InvokeSpecial(This(sClassDef), con, sClassDef.lineCol)
                  loop.break()
                }
              }
            }
            ...
            for (cons <- parent.constructors) {
              ...
              innerLoop.breakable {
                if (cons.parameters.size == astClass.superWithInvocation.args.size) {
                  invokeSpecial =
                    InvokeSpecial(This(sClassDef),
                                  cons,
                                  astClass.superWithInvocation.lineCol)
                  val parameters = cons.parameters
                  val args = astClass.superWithInvocation.args
                  parameters.indices.foreach { i =>
                    val v = parseValueFromExpression(args(i),
                                                     parameters(i).sType,
                                                     constructorScope)
          ...
          fieldLoop.breakable {
            for (param <- constructorToFillStatements.parameters) {
              val f: SFieldDef =
                sClassDef.fields.find(_.name == param.name).orNull
              if (f == null)
        ...
        case _ =>
          throw new IllegalArgumentException(
            "wrong STypeDefType " + sTypeDef.getClass)
      }
    }
  ...
  def getTypeWithAccess(mightBeClassAccess: Access,
                        imports: ListBuffer[Import]): STypeDef = {
    null
  }

  def getFieldsAndMethodsFromClass(cls: Class[_],
                                   s: STypeDef,
                                   fields: ListBuffer[SFieldDef],
                                   methods: ListBuffer[SMethodDef]): Unit = ???

  def getParameterFromClassArray(getParameterTypes: Array[Class[_]],
                                 constructorDef: SConstructorDef): Unit = ???

  def getModifierFromMember(con: Constructor[_],
                            constructorDef: SConstructorDef): Unit = ???
          ...
          typeDef match {
            ...
            case s: SClassDef =>
              ...
              if (cls != classOf[Object]) {
                s.parent = getTypeWithName(cls.getSuperclass.getName, lineCol)
                  .asInstanceOf[SClassDef]
              }
              ...
              for (con <- cls.getDeclaredConstructors) {
                ...
                getParameterFromClassArray(con.getParameterTypes,
                                           constructorDef)
            ...
            case s: SAnnoDef =>
              for (m <- cls.getDeclaredMethods) {
                ...
                annoField.sType =
                  getTypeWithName(m.getReturnType.getName, lineCol)
  ...
  private def getModifierFromClass(cls: Class[_],
                                   modifiers: ListBuffer[SModifier]): Unit = {}
  ...
  private def putNameAndTypeDef(sTypeDef: STypeDef, lineCol: LineCol): Unit = {
    if (types.contains(sTypeDef.fullName))
      throw new SyntaxException(s"duplicate type names ${sTypeDef.fullName}",
                                lineCol)
    else
