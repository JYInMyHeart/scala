...
object Ins {
  ...
  case class ANewArray(arrayType: SArrayTypeDef,
                       componentType: STypeDef,
                       count: IntValue) extends Value {
  ...
  case class CheckCast(theValueToCheck: Value,
                       requiredType: STypeDef,
                       lineCol: LineCol) extends Instruction with Value {
    override def typeOf(): STypeDef = requiredType
  }


  case class ExStore(ex: LeftValue,
                     scope: SemanticScope) extends Instruction {
    val index = scope.getIndex(ex)

    override def lineCol(): LineCol = null
  }

  case class GetClass(targetType: STypeDef,
                      classClassDef: STypeDef) extends Value {
    override def typeOf(): STypeDef = classClassDef
  }

  case class GetField(field: SFieldDef,
                      lineCol: LineCol,
                      value: Value) extends Value with Instruction {
    override def typeOf(): STypeDef = field.typeOf()
  }

  case class GetStatic(field: SFieldDef,
                       lineCol: LineCol) extends Value with Instruction {
    override def typeOf(): STypeDef = field.typeOf()
  }
  ...
  case class IfEq(condition: Value,
                  gotoIns: Instruction,
                  lineCol: LineCol) extends Instruction {

  }

  case class IfNe(condition: Value,
                  gotoIns: Instruction,
                  lineCol: LineCol) extends Instruction {

  }

  class Invoke(val invokable: SInvokable,
               val lineCol: LineCol) extends Instruction with Value {
    val arguments: ListBuffer[Value] = ListBuffer[Value]()

    override def typeOf(): STypeDef = invokable.returnType
  }

  case class InvokeDynamic(bootstrapMethod: SInvokable,
                           methodName: String,
                           args: ListBuffer[Value],
                           returnType: STypeDef,
                           indyType: Int,
                           override val lineCol: LineCol) extends Invoke(bootstrapMethod, lineCol) {

  }

  class InvokeWithTarget(target: Value,
                         invokable: SInvokable,
                         lineCol: LineCol) extends Invoke(invokable, lineCol)
    with Instruction
    with Value {

  }

  case class InvokeSpecial(target: Value,
                           override val invokable: SInvokable,
                           override val lineCol: LineCol) extends InvokeWithTarget(target, invokable, lineCol) {

  }

  case class InvokeInterface(target: Value,
                             override val invokable: SInvokable,
                             override val lineCol: LineCol) extends InvokeWithTarget(target, invokable, lineCol) {

  }

  case class InvokeStatic(override val invokable: SInvokable,
                          override val lineCol: LineCol) extends Invoke(invokable, lineCol) {

  }

  case class InvokeVirtual(target: Value,
                           override val invokable: SInvokable,
                           override val lineCol: LineCol) extends InvokeWithTarget(target, invokable, lineCol) {

  }

  case class LogicAnd(b1: Value,
                      b2: Value,
                      lineCol: LineCol) extends Value with Instruction {
    override def typeOf(): STypeDef = BoolTypeDef.get()
  }

  case class LogicOr(b1: Value,
                     b2: Value,
                     lineCol: LineCol) extends Value with Instruction {
    override def typeOf(): STypeDef = BoolTypeDef.get()
  }

  case class MonitorEnter(valueToMonitor: Value,
                          lineCol: LineCol) extends Instruction {
    var index: Int = _

    def this(valueToMonitor: Value,
             scope: SemanticScope,
             lineCol: LineCol) = {
  ...
  case class MonitorExit(enterInstruction:MonitorEnter,
                         lineCol: LineCol) extends Instruction{
  ...
  case class New(constructor:SConstructorDef,
                 lineCol: LineCol) extends Value with Instruction{
  ...
  case class NewArray(count:IntValue,
                      mode:Int,
                      storeMode:Int,
                      sType:STypeDef) extends Value{
  ...
  case class OneVarOp(value: Value,
                      op:Int,
                      sType:STypeDef,
                      lineCol: LineCol) extends Value with Instruction{
    override def typeOf(): STypeDef = sType
  ...
  case class PutField(field:SFieldDef,
                      obj:Value,
                      value: Value,
                      lineCol: LineCol) extends Instruction{
  ...
  case class PutStatic(field:SFieldDef,
                       value: Value,
                       lineCol: LineCol) extends Instruction{

  }

  case class TALoad(arr:Value,
                    index:Value,
                    modeValue:Int,
                    lineCol: LineCol) extends Value with Instruction{
