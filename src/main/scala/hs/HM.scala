...
case class Letrec(v: String, defn: SyntaxNode, body: SyntaxNode)
    extends SyntaxNode
...
object SyntaxNode {
  ...
  def nakedString(ast: SyntaxNode) = ast match {
    ...
    case l: Let =>
      "let " + l.v + " = " + string(l.defn) + " in " + string(l.body)
    case l: Letrec =>
      "letrec " + l.v + " = " + string(l.defn) + " in " + string(l.body)
  }
}
...
object TypeSystem {
  ...
  def analyse(ast: SyntaxNode, env: Env, nongen: Set[Variable]): Type =
    ast match {
...
object HindleyMilner {
  ...
  def main(args: Array[String]) {
    ...
    val myenv: TypeSystem.Env = Map.empty ++ Array(
      ...
      "cond" -> TypeSystem.Function(
        TypeSystem.Bool,
        TypeSystem.Function(var3, TypeSystem.Function(var3, var3))
      ),
