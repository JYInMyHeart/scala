...
object Queens {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println()
    (for (x <- list if x % 2 == 1; y <- list if y % 2 == 0)
      yield x + y) foreach print
    println
    list2 withFilter (_ % 2 == 1) flatMap (
      x => list2 withFilter (_ % 2 == 0) map (y => x + y)
    ) foreach print
    println
    ...
    def quickSort(list: List[Int]): List[Int] = list match {
      case Nil => Nil
      case x :: xs =>
        quickSort(xs filter (_ > x)) ++ List(x) ++ quickSort(xs filter (_ <= x))
