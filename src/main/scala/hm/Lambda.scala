...
object Lambda extends StandardTokenParsers with ImplicitConversions {
  ...
  def boolean: Parser[Term] = ("true" | "false") ^^ { s =>
    Const(s.toBoolean, Prim("Bool"))
  }
  def string: Parser[Term] = stringLit ^^ { s =>
    Const(s, Prim("String"))
  }
  def double: Parser[Term] = numericLit ^^ { s =>
    Const(s.toDouble, Prim("Double"))
  }
  ...
  def op: Parser[Term] = "(" ~> expr ~ operator ~ expr <~ ")" ^^ {
    case e1 ~ o ~ e2 => App(App(o, e1), e2)
  }

  def expr: Parser[Term] =
    variable | literal | lam | let | app | op | "(" ~> expr <~ ")"
